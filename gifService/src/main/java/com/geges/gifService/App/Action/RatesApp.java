...
public class RatesApp implements RatesInterface {
    ...
    public void updateRates() {
        ...
        this.updateCurrentRates(currTime);
        this.updatePrevRates(currTime);
    }

    private void updateCurrentRates(long timeNow) {
        if (
                this.currRates == null ||
                        !time.format(Long.valueOf(this.currRates.getTimestamp()) * 1000)
                                .equals(time.format(time))
        ) {
            this.currRates = oerClient.getLatestRates(this.appId);
        }
    }
